...
function stats() {
    $('#LPStaked').html(numberWithCommas(1000));
    $('#rPepeAvaliable').html(numberWithCommas(1000));
    $('#ETHAvaliable').html(numberWithCommas(1000));
...
$(function() {
    ...
    $('#convertLP').click(function(e) {
        e.preventDefault();
        $('#convertLPModal').modal('show');
        $('#convertRpepe').val('');
        $('#convertETH').val('');
        $('#confirmConvert').attr('disabled', '');
        $('#confirmConvert + p').addClass('d-none');
        $('#approveConvert + p').addClass('d-none');
        $('#approveConvert').addClass('d-none');
        $('.view-a').addClass('d-none');
    });


    $('#convertRpepe').keyup(function() {
        RPepeCONTRACT.methods.allowance(accounts[0], STAKINGCONTRACT_ADDRESS).call().then(allowed => {
            allowance = allowed;
            if($(this).val() !== '') {
                if ($(this).val() > allowance) {
                    $('#approveConvert + p').removeClass('d-none');
                    $('#approveConvert').removeClass('d-none');
                    $('#approveConvert + p').html(approvalMsg);
                    $('#approveConvert').find('.after').addClass('d-none');
                    $('#approveConvert').find('.before').removeClass('d-none');
                    $('#confirmConvert').attr('disabled', '');
                    $('#confirmConvert + p').addClass('d-none');
                } else {
                    $('#approveConvert').addClass('d-none');
                    ...
                    $('#confirmConvert + p').removeClass('d-none');
                }
            }else {
                $('#approveConvert').addClass('d-none');
                $('#approveConvert + p').addClass('d-none');
    ...
    $('.max-convert-button').click(function(e) {
        e.preventDefault();
        $(this).parents('.position-relative').find('input').val(RPepeBalance / 10 ** 18);
        $('#convertedLP').html(RPepeBalance / 10 ** 18);
        //$(this).parents('.position-relative').find('button#approve').removeAttr('disabled');
        RPepeCONTRACT.methods.allowance(accounts[0], STAKINGCONTRACT_ADDRESS).call().then(allowed => {
            allowance = allowed;
            if($('#convertRpepe').val() !== '') {
                if ($('#convertRpepe').val() > allowance) {
                    $('#approveConvert + p').removeClass('d-none');
                    $('#approveConvert').removeClass('d-none');
                    $('#approveConvert + p').html(approvalMsg);
                    $('#approveConvert').find('.after').addClass('d-none');
                    $('#approveConvert').find('.before').removeClass('d-none');
                    $('#approveConvert').removeAttr('disabled');
                    $('#confirmConvert').attr('disabled', '');
                    $('#confirmConvert + p').addClass('d-none');
                } else {
                    $('#approveConvert').addClass('d-none');
    ...
    $('#approveConvert').click(function(e) {
        e.preventDefault();
        ...
        RPepeCONTRACT.methods.approve(STAKINGCONTRACT_ADDRESS, RPepeBalance).send({
            ...
            $('#approveConvert').find('.before').addClass('d-none');
            $('#approveConvert').find('.after').removeClass('d-none');
            $('#approveConvert').attr('disabled', '');
            $('#approveConvert + p').html('Pending... Keep the pop up open');

            $('#approveConvert ~ .view-a').removeClass('d-none').html('View on Etherscan').attr('href', 'https://ropsten.etherscan.io/tx/' + hash);
            ...
            $('#confirmConvert').removeAttr('disabled');
            $('#confirmConvert + p').removeClass('d-none').html('Ready to stake?');
            RPepeCONTRACT.methods.allowance(accounts[0], STAKINGCONTRACT_ADDRESS).call().then(allowed => {
                allowance = allowed;
            });
            $('#approveConvert ~ .view-a').removeClass('d-none').html('<i class="fa fa-check mr-1"></i> Confirmed').attr('href', 'https://ropsten.etherscan.io/tx/' + receipt);
        });
    });


    $('#confirmConvert').click(function(e) {
        e.preventDefault();
        let formVal = [];

        $("#convertForm input").each(function() {
        ...
        STAKINGCONTRACT.methods.Stake((convert(formVal * 10 ** 18)).toString()).send({
            ...
            $('#confirmConvert').attr('disabled', '');
            $('#confirmConvert + p').removeClass('d-none').html('<i class="fa fa-check mr-1"></i> Transaction submitted');
            $('#confirmConvert ~ .view-a').removeClass('d-none').html('View on Etherscan ').attr('href', 'https://ropsten.etherscan.io/tx/' + hash);
